library("data.table")

# Loading the electric power consumption data as reading the text file
epcDT <- data.table::fread(input = "data/household_power_consumption.txt", na.strings="?")
epcDT[, Global_active_power := lapply(.SD, as.numeric), .SDcols = c("Global_active_power")]
epcDT[, dateTime := as.POSIXct(paste(Date, Time), format = "%d/%m/%Y %H:%M:%S")]

# Filter Dates for 2007-02-01 and 2007-02-02
epcDT <- epcDT[(dateTime >= "2007-02-01") & (dateTime < "2007-02-03")]

png("plot2.png", width=480, height=480)

## Plotting the diagram
plot(x = epcDT[, dateTime], y = epcDT[, Global_active_power], type="l", xlab="", ylab="Global Active Power (kilowatts)")

dev.off()